...
def demo(sentence_list, num_return_sequences, num_beams,temperature,do_sample, input_string):
    ...
    for i in range(0,num_return_sequences,1):
        ...
        BLUE_score,TER_score,ROUGE_score,SACREBLUE_score = eval(predict_text= paraphrased_text, reference_text= input_string)
        dict_in_dict[i] = {
...
def demo_parrot(sentence_list, num_return_sequences, input_string):
    ...
    if para_phrases is not None:
        for i in para_phrases:
            BLUE_score,TER_score,ROUGE_score,SACREBLUE_score = eval(predict_text= i[0], reference_text= input_string)
            dict_in_dict[1] = {
            "origin_text": f"{input_string}",
...
with st.form(key="my_form"):
    ...
    with c1:
        ModelType = st.radio(
            "Choose your model",
            ["ThanhJamieAI/ParapharseV13_10E_4B", "ThanhJamieAI/ParapharseV8_8E_4B", "ThanhJamieAI/ParapharseV11_4E_2B", "Tunner", "Parrot", "4models"],
        ...
        if ModelType == "ThanhJamieAI/ParapharseV13_10E_4B":
            ...
            def load_model():
                ...
                model = PegasusForConditionalGeneration.from_pretrained(ModelType).to(device)
        ...
        if ModelType == "4models":
            @st.cache(allow_output_mutation=True)
            def load_model():
                tokenizer_1 = PegasusTokenizer.from_pretrained("ThanhJamieAI/ParapharseV13_10E_4B")
                model_1 = PegasusForConditionalGeneration.from_pretrained("ThanhJamieAI/ParapharseV13_10E_4B").to(device)
                tokenizer_2 = PegasusTokenizer.from_pretrained("ThanhJamieAI/ParapharseV8_8E_4B")
                model_2 = PegasusForConditionalGeneration.from_pretrained("ThanhJamieAI/ParapharseV8_8E_4B").to(device)
                tokenizer_3 = PegasusTokenizer.from_pretrained("ThanhJamieAI/ParapharseV11_4E_2B")
                model_3 = PegasusForConditionalGeneration.from_pretrained("ThanhJamieAI/ParapharseV11_4E_2B").to(device)
                tokenizer_tunner = PegasusTokenizer.from_pretrained("tuner007/pegasus_paraphrase")
                model_tunner = PegasusForConditionalGeneration.from_pretrained("tuner007/pegasus_paraphrase").to(device)
                model_parrot = Parrot(model_tag="prithivida/parrot_paraphraser_on_T5", use_gpu=True)
                return model_1,tokenizer_1,model_2,tokenizer_2,model_3,tokenizer_3,model_tunner,tokenizer_tunner, model_parrot
            model_1,tokenizer_1,model_2,tokenizer_2,model_3,tokenizer_3,model_tunner,tokenizer_tunner, model_parrot = load_model()
...
if not submit_button:
    st.stop()
if ModelType == "ThanhJamieAI/ParapharseV13_10E_4B":
    output_dict = demo(sentence_list=sentence_list,num_return_sequences=num_return_sequences,num_beams=num_beams,temperature=1.5,do_sample=True,input_string=doc)
...
if ModelType == "Tunner":
    ...
    st.write(output_dict)
if ModelType == "Parrot":
    output_dict =  demo_parrot(sentence_list=doc,num_return_sequences=num_return_sequences,input_string=doc)
    st.write(output_dict)
# if ModelType == "4models":
