...
def demo(start_time, model, tokenizer, sentence_list, num_return_sequences, num_beams, temperature, do_sample, input_string):
    ...
    translated = model.generate(**batch, max_length=300, num_beams=num_beams,
...
def demo_parrot(start_time, sentence_list, num_return_sequences, input_string):
    start_time = start_time
    dict_in_dict = {}
...
with st.form(key="my_form"):
    ...
    with c2:
        doc = st.text_area(
            "Paste your text below (max 300 words)",
            height=510,
        )
...
if not submit_button:
    st.stop()
if ModelType == "ThanhJamieAI/ParapharseV13_10E_4B":
    output_dict = demo(start_time=start_time, model=model, tokenizer=tokenizer, sentence_list=sentence_list,
                       num_return_sequences=num_return_sequences, num_beams=num_beams, temperature=1.5, do_sample=False, input_string=doc)
...
if ModelType == "tuner007/pegasus_paraphrase":
    ...
    st.write(output_dict)
if ModelType == "Parrot":
    output_dict = demo_parrot(start_time=start_time,
